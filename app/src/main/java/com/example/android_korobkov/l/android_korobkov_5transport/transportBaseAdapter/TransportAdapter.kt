...
import com.example.android_korobkov.l.android_korobkov_5transport.databinding.ItemTransportBinding
...
class TransportAdapter(
    ...
    private fun getDefaultView(
        ...
        val binding: ItemTransportBinding
        val view: View

        // Если представление элемента списка еще не создано
        if (convertView == null) {
            binding = ItemTransportBinding.inflate(LayoutInflater.from(context), parent, false)
            view = binding.root
            view.tag = binding
        } else {
            binding = convertView.tag as ItemTransportBinding
            view = convertView
        }
